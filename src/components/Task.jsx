...
const Task = ({ task, index, updateTask, handleDragStart, handleDrop, handleDelete }) => {
    ...
    return (
        <div
            className={`task-enter group flex w-full min-w-0 items-center gap-2 rounded-[var(--radius-md)] border px-3 py-2 transition ${
                task.done
        ...
        >
            {!task.done ? <GripVerticalIcon className="shrink-0 cursor-grab text-slate-400" size={18} /> : null}
            <input
                type="checkbox"
                checked={task.done}
            ...
            {isEditing ? (
                <input
                    value={task.title}
                    autoFocus
                    className="w-full min-w-0 rounded-[var(--radius-sm)] bg-[var(--color-surface-muted)] px-2 py-1 text-sm text-slate-800 outline-none ring-2 ring-transparent transition focus:ring-slate-300"
                    onInput={(e) => {
                ...
                <div className="flex min-w-0 flex-1 items-center justify-between gap-2">
                    <p
                        ...
                        className={`min-w-0 flex-1 truncate text-sm ${task.done ? 'text-slate-400 line-through' : 'text-slate-700'}`}
                        title={task.title}
                    >
                        {task.title}
                    </p>
